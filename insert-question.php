...
if(empty($id)){
	$last_order = $conn->query("SELECT * FROM quiz_question where quiz_id = $qid order by order_by desc limit 1")->fetch_array()['order_by'];
	$order_by = $last_order > 0 ? $last_order + 1 : 0;
    $data = 'quques_question = "'.$question.'" ';
    $data .= ', order_by = "'.$order_by.'" ';
	$data .= ', quiz_id = "'.$qid.'" ';
	$insert_question = $conn->query("INSERT INTO quiz_question set ".$data);
	if($insert_question){
		...
		if(count($insert) == 4){
			...
			$delete = $conn->query("DELETE FROM quiz_question where quques_id =".$question_id);
		...
		$data = 'quques_question = "'.$question.'" ';
		$data .= ', quiz_id = "'.$qid.'" ';
		$update = $conn->query("UPDATE quiz_question set ".$data." where quques_id = ".$id);
		if($update){
